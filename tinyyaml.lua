-------------------------------------------------------------------------------
-- tinyyaml - YAML subset parser, EDITED by @H3xad3cimalDev
-------------------------------------------------------------------------------
...
local function encode_assert_valid_key(key)
	return not (key == nil or type(key) == "number")
end

local function encode_string(key, str)
	if not encode_assert_valid_key(key) then
		return "- \"" .. str:gsub("\n", "\\n") .. "\""
	end
	return key .. ": \"" .. str:gsub("\n", "\\n") .. "\""
end

local function encode_num(key, num)
	if not encode_assert_valid_key(key) then
		return "- " .. tostring(num)
	end
	return key .. ": " .. tostring(num)
end

local function encode_boolean(key, bool)
	return encode_num(key, bool)
end

local function encode_table(key, bool, tabbing) end

local encode_type_table = {
...
encode_table = function(key, tbl, tabbing, errors)
	tabbing = tabbing or ""
	...
	for k, v in pairs(tbl) do
		if not encode_type_table[type(v)] and not type(v) == "table" then
			throw("Unkown type: " .. type(v))
		end
		
		if type(v) == "table" then
			fstr = tabbing .. concat(fstr, encode_table(k, v, tabbing .. "  ", errors) .. "\n")
		else
			fstr = tabbing .. concat(fstr, encode_type_table[type(v)](k, v) .. "\n")
		end
	end
...
-- Converts a yaml "enviroment" to a valid yaml string
local function encode(source, errors)
	errors = errors or true
	local function throw(message)
		if errors then
			error(message)
	...
	for k, v in pairs(source) do
		...
		if type(v) == "table" then
			fstr = concat(fstr, encode_table(k, v, "  ", errors) .. "\n")
		else
			fstr = concat(fstr, encode_type_table[type(v)](k, v) .. "\n")
		end
	end
...
return {
	...
	version = 1.116, -- H3xad3cimal version
	forker = "git:@H3xad3cimalDev",
...
local function print_table(tbl, tabbing)
	tabbing = tabbing or ""
	io.write("0x" .. tostring(tbl):gsub("table: ", "") .. " = {\n")
	for k, v in pairs(tbl) do
		if type(v) == "string" then
			print(tabbing .. "   " .. k .. ": \"" .. v .. "\";")
		elseif type(v) == "number" or type(v) == "boolean" then
			print(tabbing .. "   " .. k .. ": " .. tostring(v))
		elseif type(v) == "table" then
			io.write(tabbing .. "   " .. k .. ": ")
			print_table(v, tabbing .. "   ")
		elseif type(v) == "function" then
			print(tabbing .. "   " .. k .. ": " .. tostring(v):gsub("function: ", "func: 0x"))
		end
	end
	print(tabbing .. "}")
end
